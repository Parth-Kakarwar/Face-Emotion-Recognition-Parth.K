import cv2
from deepface import DeepFace

faceCascade = cv2.CascadeClassifier(cv2.data.haarcascades + 'haarcascade_frontalface_default.xml')
cap = cv2.VideoCapture(1)

# Check if the webcam is opened correctly
if not cap.isOpened():
    cap = cv2.VideoCapture(0)

if not cap.isOpened():
    raise IOError("Cannot open webcam")

while True:
    ret, frame = cap.read()  # This will read one image from the video

    if not ret:
        print("Failed to capture frame from webcam. Exiting...")
        break

    # Analyze the frame for emotion detection, disable enforce_detection
    try:
        result = DeepFace.analyze(frame, actions=['emotion'], enforce_detection=False)
    except ValueError as e:
        print(f"Error during face analysis: {e}")
        result = [{'dominant_emotion': 'N/A'}]  # Default value if no face is detected

    if faceCascade.empty():
        print("Error loading cascade file")
    else:
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)

        # Detecting faces in the grayscale image
        faces = faceCascade.detectMultiScale(gray, 1.1, 4)

        # Drawing a rectangle around the faces
        for x, y, w, h in faces:
            cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2)

        # Display the dominant emotion on the video frame
        font = cv2.FONT_HERSHEY_SIMPLEX
        cv2.putText(frame,
                    result[0]['dominant_emotion'],
                    (50, 50),
                    font,
                    3,
                    (0, 0, 255),
                    2,
                    cv2.LINE_4)

    # Show the video with face detection and emotion display
    cv2.imshow('Demo video', frame)

    # Break the loop if 'q' is pressed
    if cv2.waitKey(2) & 0xFF == ord('q'):
        break

cap.release()
cv2.destroyAllWindows()
